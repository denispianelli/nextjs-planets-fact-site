...
export default function Saturn() {
  const [activeImage, setActiveImage] = useState(data[5].images.planet);
  ...
  function handleOverviewClick() {
    replace(`${pathname}?tab=overview`);
    setActiveImage(data[5].images.planet);
  }

  function handleStructureClick() {
    replace(`${pathname}?tab=structure`);
    setActiveImage(data[5].images.internal);
  }
  ...
  return (
    <main className="mb-4 justify-center lg:flex">
        ...
        >
          <button
            ...
            className={clsx('border-t-4 border-t-transparent', {
              'border-b-4 border-b-transparent text-white/50':
                tab !== 'overview',
              'border-saturn border-b-4': tab === 'overview',
            })}
          >
            overview
          </button>
          <button
            ...
            className={clsx('border-t-4 border-t-transparent', {
              'border-b-4 border-b-transparent text-white/50':
                tab !== 'structure',
              'border-saturn border-b-4': tab === 'structure',
            })}
          >
            structure
          </button>
          <button
            ...
            className={clsx('border-t-4 border-t-transparent', {
              'border-b-4 border-b-transparent text-white/50':
                tab !== 'geology',
              'border-saturn border-b-4': tab === 'geology',
            })}
          >
            surface
          </button>
      ...
      <article className="grid text-center lg:flex lg:w-[1110px] lg:flex-wrap">
        <div className="relative mx-auto my-[24px] flex h-[256px] w-[256px] items-center justify-center md:my-[27px] md:h-[422px]  md:w-[422px] lg:mb-[18px] lg:mt-[70px] lg:h-[666px] lg:w-[666px]">
          <Image src={activeImage} alt={data[5].name} fill />
          <Image
            src={data[5].images.geology}
            alt={`${data[5].name} Geology`}
        ...
        <div className="mx-[24px] md:mx-[40px] md:grid md:grid-cols-2 lg:mx-0 lg:flex lg:w-[350px] lg:flex-col lg:justify-center lg:justify-self-end">
          <div>
            ...
            >
              {tab === 'overview' && data[5].overview.content}
              {tab === 'structure' && data[5].structure.content}
              {tab === 'geology' && data[5].geology.content}
            </p>
            <p
            ...
            >
              ...
              <Link
                ...
                href={data[5][tab as string]?.source || '/'}
                target="_blank"
              >
                Wikipedia{' '}
                <Image
        ...
        <div className="mt-[28px] grid gap-2 uppercase md:mx-[40px] md:grid-cols-4 md:gap-[11px] lg:col-span-2 lg:m-0 lg:flex lg:justify-center lg:gap-[30px] lg:text-left">
          <div className="mx-[24px] flex justify-between border border-white/20 px-6 py-2 md:mx-0 md:flex-col lg:h-[128px] lg:w-[255px]">
            ...
            <p className="text-[20px] lg:text-[40px]">{data[5].rotation}</p>
          </div>
          <div className="mx-[24px] flex justify-between border border-white/20 px-6 py-2 md:mx-0 md:flex-col lg:h-[128px] lg:w-[255px]">
            <h4
              className={`${leagueSpartan.className} text-h4 font-bold text-white/50 lg:mt-[20px]`}
            ...
            <p className="text-[20px] lg:text-[40px]">{data[5].revolution}</p>
          </div>
          <div className="mx-[24px] flex justify-between border border-white/20 px-6 py-2 md:mx-0 md:flex-col lg:h-[128px] lg:w-[255px]">
            <h4
              className={`${leagueSpartan.className} text-h4 font-bold text-white/50 lg:mt-[20px]`}
            ...
            <p className="text-[20px] lg:text-[40px]">{data[5].radius}</p>
          </div>
          <div className="mx-[24px] flex justify-between border border-white/20 px-6 py-2 md:mx-0 md:flex-col lg:h-[128px] lg:w-[255px]">
            <h4
              className={`${leagueSpartan.className} text-h4 font-bold text-white/50 lg:mt-[20px]`}
            ...
            <p className="text-[20px] lg:text-[40px]">{data[5].temperature}</p>
          </div>
        </div>
      </article>
    </main>
