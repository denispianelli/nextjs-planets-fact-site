...
export default function Uranus() {
  const [activeImage, setActiveImage] = useState(data[6].images.planet);
  ...
  function handleOverviewClick() {
    replace(`${pathname}?tab=overview`);
    setActiveImage(data[6].images.planet);
  }

  function handleStructureClick() {
    replace(`${pathname}?tab=structure`);
    setActiveImage(data[6].images.internal);
  }
  ...
  return (
    <main className="mb-4 justify-center lg:flex">
        ...
        >
          <button
            ...
            className={clsx('border-t-4 border-t-transparent', {
              'border-b-4 border-b-transparent text-white/50':
                tab !== 'overview',
              'border-uranus border-b-4': tab === 'overview',
            })}
          >
            overview
          </button>
          <button
            ...
            className={clsx('border-t-4 border-t-transparent', {
              'border-b-4 border-b-transparent text-white/50':
                tab !== 'structure',
              'border-uranus border-b-4': tab === 'structure',
            })}
          >
            structure
          </button>
          <button
            ...
            className={clsx('border-t-4 border-t-transparent', {
              'border-b-4 border-b-transparent text-white/50':
                tab !== 'geology',
              'border-uranus border-b-4': tab === 'geology',
            })}
          >
            surface
          </button>
      ...
      <article className="grid text-center lg:flex lg:w-[1110px] lg:flex-wrap">
        <div className="relative mx-auto my-[63px] flex h-[176px] w-[176px] items-center justify-center md:my-[93px] md:h-[290px]  md:w-[290px] lg:mb-[138px] lg:mt-[158px] lg:h-[458px] lg:w-[458px]">
          <Image src={activeImage} alt={data[6].name} fill />
          <Image
            src={data[6].images.geology}
            alt={`${data[6].name} Geology`}
        ...
        <div className="mx-[24px] md:mx-[40px] md:grid md:grid-cols-2 lg:mx-0 lg:flex lg:w-[350px] lg:flex-col lg:justify-center lg:justify-self-end">
          <div>
            ...
            >
              {tab === 'overview' && data[6].overview.content}
              {tab === 'structure' && data[6].structure.content}
              {tab === 'geology' && data[6].geology.content}
            </p>
            <p
            ...
            >
              ...
              <Link
                ...
                href={data[6][tab as string]?.source || '/'}
                target="_blank"
              >
                Wikipedia{' '}
                <Image
        ...
        <div className="mt-[28px] grid gap-2 uppercase md:mx-[40px] md:grid-cols-4 md:gap-[11px] lg:col-span-2 lg:m-0 lg:flex lg:justify-center lg:gap-[30px] lg:text-left">
          <div className="mx-[24px] flex justify-between border border-white/20 px-6 py-2 md:mx-0 md:flex-col lg:h-[128px] lg:w-[255px]">
            ...
            <p className="text-[20px] lg:text-[40px]">{data[6].rotation}</p>
          </div>
          <div className="mx-[24px] flex justify-between border border-white/20 px-6 py-2 md:mx-0 md:flex-col lg:h-[128px] lg:w-[255px]">
            <h4
              className={`${leagueSpartan.className} text-h4 font-bold text-white/50 lg:mt-[20px]`}
            ...
            <p className="text-[20px] lg:text-[40px]">{data[6].revolution}</p>
          </div>
          <div className="mx-[24px] flex justify-between border border-white/20 px-6 py-2 md:mx-0 md:flex-col lg:h-[128px] lg:w-[255px]">
            <h4
              className={`${leagueSpartan.className} text-h4 font-bold text-white/50 lg:mt-[20px]`}
            ...
            <p className="text-[20px] lg:text-[40px]">{data[6].radius}</p>
          </div>
          <div className="mx-[24px] flex justify-between border border-white/20 px-6 py-2 md:mx-0 md:flex-col lg:h-[128px] lg:w-[255px]">
            <h4
              className={`${leagueSpartan.className} text-h4 font-bold text-white/50 lg:mt-[20px]`}
            ...
            <p className="text-[20px] lg:text-[40px]">{data[6].temperature}</p>
          </div>
        </div>
      </article>
    </main>
